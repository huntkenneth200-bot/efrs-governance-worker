export type IsoTimestamp = string;
export interface ErrorEnvelope {
    ok: false;
    error: {
        code: string;
        message: string;
        details?: Record<string, unknown>;
    };
}
export interface SuccessEnvelope<T> {
    ok: true;
    data: T;
}
export type ApiResponse<T> = SuccessEnvelope<T> | ErrorEnvelope;
export interface Pagination {
    limit: number;
    offset: number;
}
export interface VersionedDto {
    version: string;
}
/**
 * Generic helper to build a success envelope.
 */
export declare function success<T>(data: T): SuccessEnvelope<T>;
/**
 * Generic helper to build an error envelope.
 */
export declare function failure(code: string, message: string, details?: Record<string, unknown>): ErrorEnvelope;
//# sourceMappingURL=index.d.ts.map